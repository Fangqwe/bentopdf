...
export default defineConfig(() => {
  ...
  return {
    ...
    // ... 前面所有的配置保持不变，直到 build 部分

build: {
  rollupOptions: {
    input: {
      main: process.env.SIMPLE_MODE === 'true'
      ...
      'add-page-labels': resolve(__dirname, 'src/pages/add-page-labels.html'),
      ...
      'bates-numbering': resolve(__dirname, 'src/pages/bates-numbering.html')
    },  // <-- 确保这里有一个闭合的大括号和逗号
    output: {
